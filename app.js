...
const state = { screen: "home", loading: true, error: "", query: "", type: "all", mountains: [], courses: [], results: [], selected: null };
let toastTimer;
...
function typeButtons() {
  return [["all", "전체"], ["mountain", "산"], ["course", "걷기 코스"]].map(([value, label]) => `<button class="${state.type === value ? "active" : ""}" data-type="${value}">${label}</button>`).join("");
}
...
function searchView() {
  ...
  return `<main class="app"><header class="screen-head"><button class="back" data-action="home" aria-label="뒤로">‹</button><div><small>전국 공공데이터</small><h1>산·걷기 코스 검색</h1></div></header><section class="recommend"><aside class="filters"><span class="eyebrow" style="color:#668c78">전국 산길 찾기</span><h1>어디로<br>떠날까요?</h1><p>산 이름, 시·군 또는 걷기 코스 이름을 입력하세요.</p><div class="choice"><label>검색 종류</label><div>${typeButtons()}</div></div><p style="margin-top:30px;font-size:12px">산림청 ${state.mountains.length.toLocaleString()}건<br>두루누비 ${state.courses.length.toLocaleString()}건</p></aside><section class="results"><span class="results-label">검색 결과</span><h2>${title}</h2>${searchForm(true)}${cards}${state.results.length > 50 ? "<p>결과가 많아 처음 50건만 표시했습니다. 지역명을 함께 입력해 보세요.</p>" : ""}</section></section>${bottomNav("search")}</main>`;
}
...
function detailView() {
  ...
  const mountain = item.kind === "mountain";
  return `<main class="app"><header class="screen-head"><button class="back" data-action="search" aria-label="뒤로">‹</button><div><small>${item.source}</small><h1>${esc(item.name)}</h1></div></header><section class="detail"><div class="map-wrap"><div class="mountain-visual" style="position:absolute;inset:0"><div class="sun"></div><div class="ridge back"></div><div class="ridge"></div><span class="badge left">${esc(item.region)}</span><span class="badge right">${mountain ? "전국 산 정보" : "걷기 코스"}</span></div></div><aside class="summary"><span class="eyebrow">${mountain ? "산림청 산 정보" : "두루누비 걷기 코스"}</span><h2>${esc(item.name)}</h2><p>${esc(item.details || item.summary || "상세 설명이 제공되지 않았습니다.")}</p><div class="metrics"><div><b>${esc(item.region)}</b><span>지역</span></div><div><b>${esc(mountain ? (item.height ? `${item.height}m` : "없음") : (item.distance ? `${item.distance}km` : "없음"))}</b><span>${mountain ? "높이" : "거리"}</span></div><div><b>${esc(mountain ? (item.manager || "없음") : (item.duration || "없음"))}</b><span>${mountain ? "관리기관" : "예상시간"}</span></div><div><b>${item.source}</b><span>출처</span></div></div>${!mountain && item.travelerInfo ? `<div class="caution"><b>여행자 안내</b><span>${esc(item.travelerInfo)}</span></div>` : ""}<button class="naver-link" data-naver-search="${esc(item.name)}">N 네이버 지도에서 검색<span>›</span></button>${!mountain && item.gpxPath ? `<button class="primary" data-gpx="${esc(item.gpxPath)}">GPX 경로 열기 <span>›</span></button>` : ""}<small class="fine">산행 전 현장 통제·날씨·안전정보를 확인하세요.</small></aside></section>${bottomNav("search")}</main>`;
}
...
app.addEventListener("click", (event) => {
  ...
  if (result) { state.selected = [...state.mountains, ...state.courses].find((item) => item.id === result.dataset.result); state.screen = "detail"; return render(); }
